...
$getKelas=mysqli_query($db,"SELECT * FROM kelas WHERE id_angkatan = '$id_angkatan' && id_kelas ='$id_kelas' && id_guru='$id_Guru'");
...
?>
 ...
 <div class="col-xl-8 col-lg-7" style="margin-left: auto;    margin-right: auto;">
	...
	$sql="SELECT * FROM jadwal WHERE id_kelas = $id_kelas && mata_pelajaran != '' && hari='$hari' && id_guru=$id_guru ";
  $query=mysqli_query($db,$sql);
  $total = mysqli_num_rows($query);
  $pages = ceil($total/$halaman); 
  $getData = mysqlI_query($db,"SELECT * FROM jadwal WHERE id_kelas = $id_kelas && mata_pelajaran != '' && hari='$hari' && id_guru=$id_guru LIMIT $mulai, $halaman");
$i=0;
  ...
  <?php for ($j=1; $j<=$pages ; $j++){ ?>
    <li >
  <a class="btn" style="border:solid 1px grey ; margin:4px;" href="?halaman=<?php echo "$j&&hari=$hari&&id_kelas=$id_kelas&&id=$id_Guru&&id_angkatan=$id_angkatan"; ?>"><?php echo $j; ?></a>
  </li>
  <?php } ?>
  </ul>
</div>
...
<script>
    ...
    var id_guru=document.getElementById('guru').value;
    var id=document.getElementById('kelas').value;
    var hari=document.getElementById('hari').value;
// 	var data = $('#myForm').serialize();
// console.log(data);
      var urlID = "jadwal_detail.php?id="+id_guru+"&&id_kelas="+id+"&&hari="+hari+"&&id_angkatan="+id_angkatan;
      console.log(urlID);
        $.ajax({
