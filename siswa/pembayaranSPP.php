...
?>
 ...
 <div class="col-xl-12 col-lg-7" style="margin-left: auto;    margin-right: auto;">
	...
	$sql="SELECT * FROM siswa RIGHT JOIN pembayaran_spp ON siswa.id_siswa = pembayaran_spp.id_siswa WHERE pembayaran_spp.id_siswa='$id_siswa'";
  $query=mysqli_query($db,$sql);
  $total = mysqli_num_rows($query);
  $pages = ceil($total/$halaman); 
  $getData = mysqlI_query($db,"SELECT * FROM siswa RIGHT JOIN pembayaran_spp ON siswa.id_siswa = pembayaran_spp.id_siswa   WHERE pembayaran_spp.id_siswa='$id_siswa' LIMIT $mulai, $halaman");
$i=0;
  ...
  <?php for ($j=1; $j<=$pages ; $j++){ ?>
    <li >
  <a class="btn" style="border:solid 1px grey ; margin:4px;" href="?halaman=<?php echo "$j&&id=$id_siswa"; ?>"><?php echo $j; ?></a>
  </li>
  <?php } ?>
  </ul>
</div>
